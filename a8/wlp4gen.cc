    ...
    if(ptree->production == "procedure INT ID LPAREN params RPAREN LBRACE dcls statements RETURN expr SEMI RBRACE") {
        ...
        while(true) {
            ...
            if(pt->production == "paramlist dcl"){
                get_name_type(ptree->children[0],name,type);
...
void GenerateMainCode(struct tree *ptree, map<string, struct symbolTable *> alltable){
    //comment code
    cout << "; Main function set" << endl;
    //import
...
void GenerateMainCode2(struct tree *ptree, map<string, struct symbolTable *> alltable){
